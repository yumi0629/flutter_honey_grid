...
const Duration _kDuration = Duration(milliseconds: 500);
...
class AnimatedHoneyGrid extends StatefulWidget {
  /// Creates a scrolling container that animates items when they are inserted or removed.
  const AnimatedHoneyGrid({
    Key key,
  ...
  /// ```
  static AnimatedHoneyGridState of(BuildContext context,
      {bool nullOk = false}) {
    assert(context != null);
    assert(nullOk != null);
    final AnimatedHoneyGridState result = context
        .ancestorStateOfType(const TypeMatcher<AnimatedHoneyGridState>());
  ...
  @override
  AnimatedHoneyGridState createState() => AnimatedHoneyGridState();
}
...
class AnimatedHoneyGridState extends State<AnimatedHoneyGrid>
    with TickerProviderStateMixin<AnimatedHoneyGrid> {
  ...
  void removeItem(int index, AnimatedListRemovedItemBuilder builder,
    ...
    final int itemIndex = _indexToItemIndex(index);
    print(
        'index = $index , itemIndex = $itemIndex , _itemsCount = $_itemsCount');
    assert(itemIndex >= 0 && itemIndex < _itemsCount);
  ...
  void addAll(int itemCount, {Duration duration = _kDuration}) {
  ...
  Widget build(BuildContext context) {
    return HoneyGridView.count(
      ...
      children: List.generate(_itemsCount, (index) {
